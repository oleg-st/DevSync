...
public abstract class AgentStarter(ILogger logger) : IDisposable
{
    ...
    protected void Initialize()
    {
        ...
        Debug.Assert(ExcludeList != null);

        var sw = SlimStopwatch.StartNew();
        var response = SendCommandInternal<InitResponse>(new InitRequest(Logger)
        {
            AgentOptions = new AgentOptions(DestPath, ExcludeList)
        });
        if (!response.Ok)
        {
    ...
    {
        Debug.Assert(PacketStream != null);
        CancellationTokenSource.Token.ThrowIfCancellationRequested();
        try
        {
