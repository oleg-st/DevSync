    ...
    : ISshStarter
{
    ...
    private void InitEncryption(ConnectionInfo connectionInfo)
    {
        ...
        * https://security.stackexchange.com/questions/180544/is-there-a-list-of-weak-ssh-ciphers         */
        ...
        connectionInfo.Encryptions["arcfour256"]
            = new CipherInfo(256, (key, _) => new Arc4Cipher(key, true));
        connectionInfo.Encryptions["arcfour"]
            = new CipherInfo(128, (key, _) => new Arc4Cipher(key, false));
        connectionInfo.Encryptions["arcfour128"]
            = new CipherInfo(128, (key, _) => new Arc4Cipher(key, true));
    ...
    public void Connect()
    {
        try
        {
            ...
            {
                OnConnectError?.Invoke(this, new SshStarterErrorEventArgs(args.Exception.Message));
            };
            lock (this)
            {
