...
namespace DevSync
{
    public class SshRsaKeyGenerator
    {
        ...
        // https://en.wikipedia.org/wiki/Key_size#cite_note-16
        public SshRsaKeyGenerator(int keySize = 2048)
        {
        ...
        {
            if (_rsaCryptoServiceProvider.PublicOnly) throw new ArgumentException("CSP does not contain a private key", nameof(_rsaCryptoServiceProvider));
            var parameters = _rsaCryptoServiceProvider.ExportParameters(true);
        ...
        public string ToRfcPublicKey(string comment)
        {
            ...
            byte[] n = _rsaCryptoServiceProvider.ExportParameters(false).Modulus;
            byte[] e = _rsaCryptoServiceProvider.ExportParameters(false).Exponent;
            ...
            ms.Flush();
            var buffer64 = Convert.ToBase64String(ms.ToArray());
            return $"ssh-rsa {buffer64} {comment}";
        }

        public string ToPublicKey()
        {
            var parameters = _rsaCryptoServiceProvider.ExportParameters(false);
            using var stream = new MemoryStream();
            var writer = new BinaryWriter(stream);
            writer.Write((byte)0x30); // SEQUENCE
            using (var innerStream = new MemoryStream())
