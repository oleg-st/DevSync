...
public class CommandRunner(ILogger logger)
{
    private string? _path;
    private bool _initialized;
    private readonly FileMaskList _excludeList = new();
    ...
    public Packet Run(Packet request)
    {
        ...
        {
            return new ErrorResponse(logger) { Message = ex.Message, Recoverable = ex.Recoverable, NeedToWait = ex.NeedToWait };
        }
        catch (Exception ex)
        {
            return new ErrorResponse(logger) { Message = ex.Message };
        }
    }
    ...
    protected ScanResponse RunScan()
    {
        ...
        var scanResponse = new ScanResponse(logger);
        var scanDirectory = new ScanDirectory(logger, _excludeList);
    ...
    {
        Debug.Assert(request.AgentOptions != null);
        ...
        _initialized = true;
        return new InitResponse(logger) { Ok = true };
    }
    ...
    protected ApplyResponse RunApply(ApplyRequest applyRequest)
    {
        ...
        return new ApplyResponse(logger) { Result = applyRequest.ReadAndApplyChanges(_excludeList) };
    }
}